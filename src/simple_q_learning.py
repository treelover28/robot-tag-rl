...
class Simple_Q_Learning_Agent:
    def __init__(self, agent_type, learning_rate, discount_factor, substates_ratings_list, actions_list, get_agent_state_function, agent_take_action_function, get_game_information):
        ...
        self.agent_training_type = "Q-Learning"
        self.learning_rate = learning_rate
        self.discount_factor = discount_factor
        # to completely separate Q-Learning algorithm file and game file
        # the game file must dictate its own implementation of methods:
        # - to get the agent's state
        # - to make agent carry out an action
        # - how an agent can get general game information like (distance_from_wall, the other agent's positions, ... so on)
    ...
    def get_policy(self, state_dictionary, verbose = True, epsilon = 1.0):
        """ 
        ...
        """
        if state_dictionary is not None and self.q_table is not None:
            # since the Q_Table takes in a tuple as the key
            # we must convert the state dictionary into a tuple
            state = self.get_state_tuple_from_dictionary(state_dictionary, ["Front", "Upper Left", "Upper Right", "Lower Left", "Lower Right","Opponent Position"])
    ...
    def learn(self, epsilon, time_to_apply_action = 0.33):
        ...
        chosen_action = self.get_policy(current_state, verbose = True, epsilon= epsilon)
        ...
        current_state_tuple = self.get_state_tuple_from_dictionary(current_state)
        new_state_tuple = self.get_state_tuple_from_dictionary(new_state)

        best_action = self.get_policy(new_state, verbose=False, epsilon = 1.0)
        self.q_table[current_state_tuple][chosen_action] += self.learning_rate*(reward + \
                        self.discount_factor * self.q_table[new_state_tuple][best_action] - self.q_table[current_state_tuple][chosen_action])    
